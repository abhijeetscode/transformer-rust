...
use config::CONTEXT_SIZE;
use config::device;
use data_loader::{get_batch, get_train_test_data};
use rand::seq::SliceRandom;
use tiktoken::get_encoding;
fn evaluate_model(gpt_model: &GPTModel, data: &Vec<u32>) -> Result<f32> {
    ...
    for chunk in starts.chunks(3) {
        ...
        let logits: Tensor = gpt_model.forward(&xs)?;
...
fn main() -> Result<()> {
    ...
    for epoch in 0..max_epochs {
        ...
        let val_loss: f32 = evaluate_model(&gpt_model, &test_data)?;
